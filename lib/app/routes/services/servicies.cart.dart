...
import 'package:newhonekapp/ui-kit/widgets/cards/shop.services.card.dart';
import 'package:newhonekapp/ui-kit/widgets/primary_button.dart';
import 'package:translator/translator.dart';

class ShopCartServices extends StatefulWidget {
  final List<ServicesModel> yourCart;
  const ShopCartServices({super.key, required this.yourCart});

  @override
  createState() => _ShopCartServices();
}

class _ShopCartServices extends State<ShopCartServices> {
  late double price = 0;
  ...
  Future<String> payment_intents(
      int? amount, String? currency, String? paymentMethodTypes) async {
    setState(() {
      isLoading = true;
    });
    ...
    var body = data;
    String token = currentBooking.value[0].key_secret != '' ? currentBooking.value[0].key_secret : '';
  ...
  Future<void> confirmPayment() async {
    try {
      ...
      for (var item in currentServicesCart.value) {
        await addShopServices({
          "title": item.name,
          "price": item.price,
          "image": item.image,
          "product_id": item.id
